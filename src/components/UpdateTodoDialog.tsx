...
const UpdateTodoDialog = (props: Props) => {
    ...
    //* callback for updating the current todo state in localStorage
    function updateHandler(todo:TodoTypes){
           
    }

    return (
        <Transition appear show={modalContext?.isUpdateOpen} as={Fragment}>
            <Dialog as="div" className="relative z-10" onClose={()=>modalContext?.handleUpdateClose()}>
                <Transition.Child
                    as={Fragment}
                        ...
                        >
                            <Dialog.Panel className="w-full max-w-md p-6 -mt-[6.5rem] overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                                ...
                                <div className="flex justify-end mt-6">
                                    <button
                                        onClick={()=>updateHandler(context?.selectedTodo)}
                                        type="button"
